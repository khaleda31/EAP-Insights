class ConnectedScatterSocial {
    ...
    drawChart(grouped, countries) {
        ...
        card.append("h3")
            ...
            .style("font-family", "Playfair Display, Georgia, serif")
            .style("font-weight", "600").style("font-size", "18px").style("color", "#1e293b")
            .text(`${xLabel} vs ${yLabel} Over Time`);

        card.append("p")
            .style("margin", "0 0 16px 0")
            .style("font-family", "Playfair Display, Georgia, serif")
            .style("font-size", "13px").style("color", "#64748b")
            .text("Social indicators. Lines connect each country's data points over time.");
        ...
        // Tooltip
        d3.selectAll(".scatter-soc-tooltip").remove();
        const tooltip = d3.select("body").append("div")
            .attr("class", "scatter-soc-tooltip")
        ...
        const line = d3.line()
            .x(d => xScale(d[xCol])).y(d => yScale(d[yCol]))
            .curve(d3.curveMonotoneX);

        const countryGroups = g.selectAll(".country-group")
            .data(Array.from(grouped)).enter().append("g").attr("class", "country-group");
            ...
            g.selectAll("circle")
                .on("mouseover", function (event, d) {
                    ...
                    tooltip.html(`
                        ...
                        <span style="color:#94a3b8">${xLabel}:</span> ${ConnectedScatterSocial.fmt(d[xCol])}<br/>
                        <span style="color:#94a3b8">${yLabel}:</span> ${ConnectedScatterSocial.fmt(d[yCol])}
